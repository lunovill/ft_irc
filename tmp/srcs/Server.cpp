...
Server::Server(int port, std::string password) : _port(port), _client_count(0),  _password(password) {
	...
	Server::_cmd_list[JOIN] = &Server::_join;
...
// 	int	current_fd =  _current_client->first;
// 	Client &current = _current_client->second;
...
bool Server::_join(std::string &cmd) {
    ...
    for (unsigned int i = 0; i < names.size(); i++) {
        ...
        while (!channel_found && it != _channels.end()) {
            if (it->name == names[i]) {
                channel_found = true;
            } else {
                ++it;
        ...
        if (it->clients.count(current_fd)) {
            //erreur
            return false;
        }
...
int Server::_commandFind(std::string &cmd) const {
	...
	for (unsigned int i = 0; i < NB_OF_CMDS; i++) {
		...
		if (res == _cmd_str[i] && (!cmd[res.length()] || (cmd[res.length()] == '\n' || cmd[res.length()] == ' ' || cmd[res.length()] == '\r'))) {
			if (_current_client->second.is_register() == -1 && res == "NICK")
...
{	
	for (unsigned int i = 0; i < tabstr.size(); i++) {
		...
		if (_current_client->second.is_register() == 1) {
			...
			send(_current_client->first, "002\r\n", 5, 0);
			send(_current_client->first, "003\r\n", 5, 0);
			send(_current_client->first, "004\r\n", 5, 0);
